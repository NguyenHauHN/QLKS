...
{ 
    public partial class Phong : Form
    {
        ...
        {
            var gia = new QuanLyKhachSan.Model.GiaPhong();
            var phong = new QuanLyKhachSan.Model.Phong();
            phong.TenPhong = textBox1.Text;
            string malp = cmbLoaiPhong.Text.TrimEnd();
            var loaiphong = db.LoaiPhongs.Where(p => p.TenLP == malp).ToList();
            gia.MaLP = loaiphong[0].MaLP.TrimEnd();
            string kieu = cmbKieuPhong.Text.TrimEnd();
            var Kieuphong = db.KieuPhongs.Where(p => p.TenKP == kieu).ToList();
            gia.MaKP = Kieuphong[0].MaKP.TrimEnd();
            gia.Gia = int.Parse(textBox2.Text);
            phong.MotaPhong = txtMota.Text;
            db.Phongs.Add(phong);
            db.GiaPhongs.Add(gia);
            db.SaveChanges();
